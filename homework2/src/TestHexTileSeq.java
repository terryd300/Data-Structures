...
public class TestHexTileSeq extends LockedTestCase {
	...
	public void test() {
		...
		// In the following, if a HexTile index is called for ("ix"), answer:
		// 0 if the result will be null
		// 1 if the result will be the HexTile b1
		// 2 if the result will be the HexTile b2
		// 3,4,5 etc.
	...
	private void testcont(HexTileSeq s) {
		s.advance();
		assertEquals(3,ix(()->s.getCurrent()));
		// At this point s is [clone(b2),b1,*b3,null] with * marking current
		HexTileSeq s2 = new HexTileSeq();
		// In the following, if a HexTile index is called for ("ix"), answer:
		// 0 if the result will be null
		// 1 if the result will be the HexTile b1
		// 2 if the result will be the HexTile b2, etc for 3, 4, 5
		// -1 if the code will crash with an exception
	...
	private void testremove(HexTileSeq s) {
