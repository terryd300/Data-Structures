...
public class HexTileSeq implements Cloneable
{
	...
	private boolean wellFormed() {
		...
		for (Node p = head; p != null; p=p.next) {
			if (p == precursor) foundpc = true;
			if (p == tail) foundt = true;
			++count;
		}
		if (manyNodes != count) return report("manyItems is " + manyNodes + ", not matching actual count=" + count);
		if (tail == null && count > 0) return report("tail is null when the list isn't empty");
		if (tail != null && !foundt) return report("ail points to a node not in the list");
		if (tail != null && tail.next != null) return report("tail is not the last element");
		if (precursor != null && !foundpc) return report("precursor not in list");
		if (precursor == null && cursor != head) return report("cursor is not head though precursor is null");
		if (precursor != null && precursor.next != cursor) return report("cursor is not next node after precursor");
		// #)
