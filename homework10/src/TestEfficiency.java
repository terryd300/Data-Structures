...
public class TestEfficiency extends TestCase {
	...
	public void testRowLong() {
		testLong(hb.row(3), (i) -> makeHexTile(i, 3), false);
		hb.clear();
	}
	...
	public void testRowContains() {
		testContains(hb.row(2), (i)->makeHexTile(i, 2));
		hb.clear();
	}
	...
	public void testRowRemove() {
		testRemove(hb.row(0), (i) ->makeHexTile(i, 0));
		hb.clear();
	}
	...
	public void testRowIsEmpty() {
		...
		for (int p = POWER; p > 0; --p) {
			int diff = (1 << (p+1));
			for (int i=(1 << p); i < max; i += diff) {
				hb.add(new HexTile(Terrain.LAND,new HexCoordinate(i,BitUtil.reverseNybble(total % 16))));
				++total;
			}
		}
		assertEquals(1<<POWER,hb.size()+1);
		for (int i=0; i < max; ++i) {
			assertFalse(hb.row(i%16).isEmpty());
		}
